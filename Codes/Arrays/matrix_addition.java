...
public class matrix_addition {
  public static void main(String[] args) {
    ...
    for (int i = 0; i < rows; i++)
      for (int j = 0; j < columns; j++) firstMatrix[i][j] = input.nextInt();

    System.out.println("ENTER THE VALUES OF SECOND MATRIX");
    // input block for second matrix
    for (int i = 0; i < rows; i++)
      for (int j = 0; j < columns; j++) secondMatrix[i][j] = input.nextInt();

    // adding two matrices
    int[][] sum = new int[rows][columns];
    for (int i = 0; i < rows; i++)
      for (int j = 0; j < columns; j++) sum[i][j] = firstMatrix[i][j] + secondMatrix[i][j];
