...
public class legendre_prime_counter {
  public static void main(String[] args) {
    legendre_prime_counter counter = new legendre_prime_counter(1000000000);
  ...
  public legendre_prime_counter(int limit) {
  ...
  private static List<Integer> generatePrimes(int limit) {
    ...
    for (int p = 3, s = 9; s < limit; p += 2) {
      if (sieve[p >> 1]) {
        for (int q = s; q < limit; q += p << 1) sieve[q >> 1] = false;
      }
      s += (p + 1) << 2;
    }
