...
public class mersenne_factors {
  ...
  public static BigInteger findFactorMersenneNumber(int primeP) {
    ...
    // There are more complicated ways of getting closer to sqrt(), but not that important here, so
    // go with simple
    ...
    while ((possibleFactor = possibleFactor.add(twoP)).compareTo(maxFactor) <= 0) {
      ...
      // "Furthermore, q must be 1 or 7 mod 8". We know it's odd due to the +1 done above, so bit 0
      // is set. Therefore, we only care about bits 1 and 2 equaling 00 or 11
  ...
  public static void main(String[] args) {
    for (int p = 1; p <= 50; p++) checkMersenneNumber(p);
    Scanner input = new Scanner(System.in);
    System.out.println("Enter a Mersenne Number: ");
    int number = input.nextInt();
    checkMersenneNumber(number);
    input.close();
    return;
  }
}
