...
public class sieve_of_sundaram {
  ...
  static int SieveOfSundaram(int n) {
    /*
     * In general Sieve of Sundaram, produces primes smaller than (2*x + 2) for a number given
     * number x. Since we want primes smaller than n, we reduce n to half
     */
    int nNew = (n - 1) / 2;
    ...
    for (int i = 1; i <= nNew; i++)
      for (int j = i; (i + j + 2 * i * j) <= nNew; j++) marked[i + j + 2 * i * j] = true;
